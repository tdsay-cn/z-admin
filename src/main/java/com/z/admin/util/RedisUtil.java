...
public class RedisUtil {
     ...
     * @param
     */
    public void del(RedisKeyEnum redisKeyEnum, String key) {
        this.del(redisKeyEnum.getKey() + key);
    }
    ...
    /**
     * 从 Redis List 中删除指定条件的元素
     *
     * @param redisKeyEnum key枚举
     * @param clazz        目标类类型
     * @param filter       删除条件（返回true表示删除）
     * @param <T>          泛型类型
     * @return 是否成功
     */
    public <T> Boolean lRemoveIf(RedisKeyEnum redisKeyEnum, Class<T> clazz, java.util.function.Predicate<T> filter) {
        try {
            List<T> list = lGetAll(redisKeyEnum, clazz);
            if (DataUtils.isEmpty(list)) {
                return true;
            }
            ...
            redisTemplate.delete(redisKeyEnum.getKey());

            if (!DataUtils.isEmpty(filteredList)) {
                lSetAll(redisKeyEnum, filteredList);
            }
            return true;
        } catch (Exception e) {
            log.error("redis lRemoveIf error , key : {}", redisKeyEnum.getKey(), e);
            return false;
        }
    }
}
